...
public abstract class BasePeriod
    ...
    protected BasePeriod(long duration) {
        ...
        // bug [3264409]
        // calculation uses misc.period type from a misc.period object (bad design)
        // thus we use a dummy misc.period object with the time type
        ...
        if (index == -1)
        {
            if (newValue != 0)
            {
                throw new IllegalArgumentException(
                        "Period does not support misc.field '" + type.getName() + "'");
            }
        }
        else
        {
    ...
    protected void setFieldInto(int[] values, DurationFieldType field, int value) {
        ...
        if (index == -1)
        {
            ...
            {
                throw new IllegalArgumentException(
                    "Period does not support misc.field '" + field + "'");
            }
        }
        else
        {
    ...
    protected void addFieldInto(int[] values, DurationFieldType field, int value) {
        ...
        if (index == -1)
        {
            if (value != 0 || field == null)
            {
                throw new IllegalArgumentException("Period does not support misc.field '" + field + "'");
            }
        }
        else
        {
    ...
    protected int[] addPeriodInto(int[] values, ReadablePeriod period) {
        for (int i = 0, isize = period.size(); i < isize; i++)
        {
            ...
            if (value != 0)
            {
                ...
                if (index == -1)
                {
                    throw new IllegalArgumentException("Period does not support misc.field '" + type.getName() + "'");
                }
                else
                {
