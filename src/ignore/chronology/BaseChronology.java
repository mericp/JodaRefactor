...
import ignore.datetime.UnsupportedDateTimeField;
...
public abstract class BaseChronology
    ...
    public void validate(ReadablePartial partial, int[] values) {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                throw new IllegalFieldValueException(field.getType(), value, field.getMinimumValue(), null);
            }

            if (value > field.getMaximumValue())
            {
                throw new IllegalFieldValueException (field.getType(), value, null, field.getMaximumValue());
            }
        }
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                throw new IllegalFieldValueException(field.getType(), value, field.getMinimumValue(partial, values), null);
            }

            if (value > field.getMaximumValue(partial, values))
            {
                throw new IllegalFieldValueException (field.getType(), value, null, field.getMaximumValue(partial, values));
            }
        }
    }
    ...
    public long add(long instant, long duration, int scalar) {
        ...
        else
        {
            ...
            added = FieldUtils.safeAdd(instant, add);
        }
