...
public class ISODateTimeFormat {
    ...
    /**
     * Returns a generic ISO misc.datetime parser which parses either a date or a time or both.
     * <p>
     * The returned formatter can only be used for parsing, printing is unsupported.
     * <p>
     * The parser is strict by default, thus time string {@code 24:00} cannot be parsed.
     ...
     * It accepts formats described by the following syntax:
     * <pre>
     * misc.datetime          = time | date-opt-time
     * time              = 'T' time-element [offset]
    ...
    /**
     * Returns a generic ISO misc.datetime parser where the date is mandatory and the time is optional.
     * <p>
     * The returned formatter can only be used for parsing, printing is unsupported.
     * <p>
     ...
     * The returned formatter can only be used for parsing, printing is unsupported.
     * <p>
     * This parser only parses misc.local datetimes.
     * This parser is initialised with the misc.local (UTC) time zone.
     * The parser is strict by default, thus time string {@code 24:00} cannot be parsed.
     * <p>
     * It accepts formats described by the following syntax:
     * <pre>
     * misc.datetime          = date-element ['T' time-element]
     * date-element      = std-date-element | ord-date-element | week-date-element
     * std-date-element  = yyyy ['-' MM ['-' dd]]
     * ord-date-element  = yyyy ['-' DDD]
     * week-date-element = xxxx '-W' ww ['-' e]
    ...
    static final class Constants {
        ...
        dpe = dateElementParser(), // date parser element
                ...
                ldp = localDateParser(), // misc.local date parser
                tp = timeParser(),  // time parser
                ltp = localTimeParser(), // misc.local time parser
                dtp = dateTimeParser(), // date time parser
                dotp = dateOptionalTimeParser(), // date optional time parser
                ldotp = localDateOptionalTimeParser(); // misc.local date optional time parser
