...
class BasicMonthOfYearDateTimeField extends ImpreciseDateTimeField {
    ...
    public long getDifferenceAsLong(long minuendInstant, long subtrahendInstant) {
        ...
        if (minuendDom == iChronology.getDaysInYearMonth(minuendYear, minuendMonth))
        {
            ...
            if (subtrahendDom > minuendDom)
            {
                ...
                // fields from within a misc.field.
    ...
    public boolean isLeap(long instant) {
        ...
        leap = iChronology.isLeapYear(thisYear) && (iChronology.getMonthOfYear(instant, thisYear) == iLeapMonth);
    ...
    public int[] add(ReadablePartial partial, int fieldIndex, int[] values, int valueToAdd) {
        ...
        else
        {
            ...
            {
                // month is largest misc.field and being added to, such as month-day
                int curMonth0 = values[0] - 1;
                int newMonth = ((curMonth0 + (valueToAdd % 12) + 12) % 12) + 1;

                added = set(partial, 0, values, newMonth);
            }
            else
            {
                if (DateTimeUtils.isContiguous(partial))
                {
                    ...
                    added = iChronology.get(partial, instant);
                }
                else
                {
                    added = super.add(partial, fieldIndex, values, valueToAdd);
                }
            }
        }
